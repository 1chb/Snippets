...
handlers env = get env :<|> \form -> Greeting.add env form >> redirectTo [] (path Main) -- TODO ?
...
input :: Html ()
input = form_ [method_ "post", action_ $ path Main <> "/add", onsubmit_ "return validateForm()"] $ do
